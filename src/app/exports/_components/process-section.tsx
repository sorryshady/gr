...
import { EXPORTS_PROCESS_CONTENT } from '@/lib/constants'
        ...
        <div>
          <Copy className="flex max-w-4xl flex-col gap-4 text-center">
            <h3 className="text-primary/50 text-lg font-medium tracking-wide md:text-2xl">
              {EXPORTS_PROCESS_CONTENT.heading}
            </h3>
            <h2 className="text-primary text-4xl font-medium md:text-5xl">{EXPORTS_PROCESS_CONTENT.headLine}</h2>
            <p className="text-primary/80 text-base font-medium md:text-lg">{EXPORTS_PROCESS_CONTENT.description}</p>
          </Copy>
        </div>
        <div className="mx-auto mt-10 grid w-full max-w-7xl grid-cols-1 gap-5 p-0 md:grid-cols-2 md:p-5 lg:grid-cols-3">
          {EXPORTS_PROCESS_CONTENT.process.map((value, index) => (
            <FadeIn
              key={value.title}
              delay={index * 0.1}
